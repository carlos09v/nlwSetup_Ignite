...
const NewHabitForm = () => {
  ...
  const handleToggleWeekDay = (weekDay: number) => {
    // percorrer a lista, ver se o dia da semana ja ta la dentro
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <input className="p-4 rounded-lg mt-3  bg-zinc-800 text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-violet-600 focus:ring-offset-2 focus:ring-offset-zinc-900" type="text" name="title" id="title" placeholder="ex.: Exercícios, dormir bem, etc..." autoFocus onChange={e => setTitle(e.target.value)} value={title} />

      <label className="font-semibold leading-tight mt-4">Qual a recorrência ?</label>
      <div className="flex flex-col gap-2 mt-3">
        {availableWeekDays.map((weekDay, i) => (
          <Checkbox.Root checked={weekDays.includes(i)} onCheckedChange={() => handleToggleWeekDay(i)} key={weekDay} className='flex items-center gap-3 group focus:outline-none'>
